...
import { Sun, ChevronRight, SlidersHorizontal } from 'lucide-react'
...
async function getSunglasses(params: Record<string, string | string[] | undefined>) {
  ...
  const where: Record<string, unknown> = {
    ...
    OR: [
      ...
      { category: { parent: { slug: 'sunglasses' } } },
  ...
  const [products, total, categories, brands] = await Promise.all([
    ...
    prisma.category.findMany({
      where: { OR: [{ slug: 'sunglasses' }, { parent: { slug: 'sunglasses' } }] },
      orderBy: { name: 'asc' },
    }),
...
export default async function SunglassesPage({ searchParams }: Props) {
  const params = await searchParams
  const data = await getSunglasses(params)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-amber-600 to-orange-500 text-white py-16">
        <div className="container text-center">
          <Sun className="h-16 w-16 mx-auto mb-4 opacity-80" />
          <h1 className="text-4xl md:text-5xl font-display font-bold mb-4">
            Sunglasses
          </h1>
          <p className="text-lg text-amber-100 max-w-2xl mx-auto">
            Protect your eyes in style with our collection of premium sunglasses.
            UV400 protection and polarized options available.
          </p>
        </div>
      </div>
      ...
      <div className="container py-4">
        <nav className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span className="text-gray-900">Sunglasses</span>
        </nav>
      </div>
          ...
          <div className="flex-1">
            ...
            {data.products.length > 0 ? (
                      ...
                      <div className="aspect-square relative bg-gray-100">
                        {product.images[0] ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center">
                            <Sun className="h-16 w-16 text-gray-300" />
                          </div>
                        )}
              ...
              <div className="bg-white rounded-xl border shadow-sm p-12 text-center">
                ...
                <Link href="/sunglasses" className="btn-primary">
                  Clear Filters
                </Link>
              </div>
            )}
